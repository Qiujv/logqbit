...
许多区域都提供与当前内容相关的右键菜单：可试试右键点击表头、记录、数据列、常量、文件或图片，以及绘图区域，探索可用操作。

## 浏览记录

Browser 左侧是记录列表，右侧是当前记录的详情。含有 `metadata.json` 的直接子目录会被识别为记录。单击表头可排序；右键表头可显示创建时间、创建机器和绘图轴等列，也可筛选是否显示星标或回收站记录。
...
- `Const.`：显示实验常量和配置。
- `Data`：显示表格数据。较大的表格先显示一部分行，点击 `Show More Rows` 继续加载；右键列还可查看该列的不同取值，或者导出数据为csv。
- `Plot`：根据数据列绘制曲线或二维色图。
...
图中可用鼠标缩放和拖动。右键图像可保存 PNG 或切换对数坐标；1D 图还可调整 marker 的 Small、Medium 或 Large 尺寸。`copy` 或 `Ctrl+C` 可复制当前图像。启用 `cursor` 后，单击图像可移动 cursor；移动鼠标时会显示半透明预览和坐标。在二维图中，cursor 还会显示横、纵截面。
