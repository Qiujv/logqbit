...
from logqbit import cli_shortcuts
...
def test_browser_demo_dispatch(monkeypatch) -> None:
    monkeypatch.setattr("logqbit.cli_demo.create_example_data", lambda: 17)
...
def test_shortcuts_create_only_browser_shortcut(
    ...
    monkeypatch.setattr(cli_shortcuts, "_browser_icon", lambda: icon_path)
    monkeypatch.setattr(cli_shortcuts.subprocess, "run", run)

    assert cli_shortcuts.create_shortcuts(tmp_path) == 0
